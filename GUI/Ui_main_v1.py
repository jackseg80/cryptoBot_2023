...
# Créer une instance de QMainWindow et la configurer avec l'interface utilisateur
window = QMainWindow()
ui = Ui_MainWindow()
ui.setupUi(window)

def show_selected_strategy():
    # Obtenir le texte de l'élément sélectionné
    selected_strategy = ui.comboBox1.currentText()
    
    # Afficher un message box avec le texte de l'élément sélectionné
    QMessageBox.information(window, "Stratégie sélectionnée", selected_strategy)

# Connecter le signal accepted du groupe de boutons à la fonction show_selected_strategy
ui.BGroup.accepted.connect(show_selected_strategy)

# Connecter le signal rejected du groupe de boutons à la méthode close de la fenêtre
ui.BGroup.rejected.connect(window.close)
